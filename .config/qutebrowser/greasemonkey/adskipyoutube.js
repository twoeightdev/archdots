...
// @license     AGPLv3
// @author      jcunews
// @description Disable YouTube video & screen based ads at home page, and right
...
((window, fn) => {
    ...
    fn = (a, ipse, haia, hca, rpo, et) => {
        ...
        if((a = document.scripts[document.scripts.length - 1]) && (a
                .id === "dyvaUjs")) a.remove();

        et = window.InstallTrigger ? "beforescriptexecute" :
        "message"; //Firefox workaround
        ...
        JSON.parse = function(a) {
            ...
            if(rpo) {
                ...
                try {
                    if(a.forEach) {
                        a.forEach((p, a) => {
                            if(p.player && p.player.args &&
                                p.player.args
                                .player_response) {
                                a = p
                                .player_response_; //JSON.parse_dyva(p.player_response);
                                patchPlayerResponse(a);
                                p.player_response = JSON
                                    .stringify(a);
                            } else if(p.playerResponse) {
                                patchPlayerResponse(p
                                    .playerResponse);
                            }
                        });
                ...
                a = JSON.parse_dyva(a);
                if(a.playerResponse) patchPlayerResponse(a
                    .playerResponse);
            }
            return a;
        };
        ...
        window.fetch = function(u) {
            if(u) {
                if(u.substr && /\/v1\/player\/ad_break/.test(u))
                    return new Promise(() => {});
                if(u.url && u.url.substr && /\/v1\/player\/ad_break/
                    .test(u.url)) return new Promise(() => {});
            }
        ...
        Response.prototype.json = function() {
            ...
            p.then = function(fn) {
                var fn_ = fn;
                fn = function(j) {
                    if(/\/v1\/player\?/.test(rs.url)) rpo =
                        j;
                    if("function" === typeof fn_) return fn_
        ...
        window.XMLHttpRequest.prototype.open_dyva = window
            .XMLHttpRequest.prototype.open;
        window.XMLHttpRequest.prototype.open = function(mtd, url) {
            if(!(/get_midroll_info/).test(url) && !((/^\/watch/)
                    .test(location.pathname) && (/get_video_info/)
                    .test(url))) {
                this.url_dyva = url;
                return this.open_dyva.apply(this, arguments);
            }
        };
        window.XMLHttpRequest.prototype.addEventListener_dyva = window
            .XMLHttpRequest.prototype.addEventListener;
        window.XMLHttpRequest.prototype.addEventListener = function(typ,
            fn) {
            if(typ === "readystatechange") {
                ...
                fn = function() {
                    ...
                    if((this.readyState === 4) && (
                            /\/watch\?|get_video_info/).test(
                            this.url_dyva)) {
                        rpo = JSON.parse_dyva(this
                        .responseText);
                        try {
                            rpo.forEach(p => {
            ...
            return this.addEventListener_dyva.apply(this,
            arguments);
        };

        window.Node.prototype.appendChild_dyva = window.Node.prototype
            .appendChild;
        window.Node.prototype.appendChild = function(node) {
            var a;
            if(!ipse && (a = document.querySelector(
                    'ytd-watch-flexy')) && (a = a.constructor
                    .prototype) && a.isPlaShelfEnabled_) {
                a.isPlaShelfEnabled_ = () => false;
                ipse = true;
            }
            if((!hca || !haia) && (a = document.querySelector(
            ...
            if(node.querySelector && (a = node.querySelector(
                    '.ytp-ad-skip-button'))) a.click();
        ...
        function patchPlayerResponse(playerResponse) {
            ...
            if(playerResponse.adPlacements) playerResponse
                .adPlacements = [];
            if(playerResponse.adSlots) playerResponse.adSlots = [];
            if(playerResponse.auxiliaryUi?.messageRenderers
                ?.bkaEnforcementMessageViewModel) {
                delete playerResponse.auxiliaryUi.messageRenderers
                    .bkaEnforcementMessageViewModel;
                if(!Object.keys(playerResponse.auxiliaryUi
                        .messageRenderers).length) {
                    delete playerResponse.auxiliaryUi.messageRenderers;
                ...
                Object.defineProperty(playerResponse, "videoDetails", {
                    ...
                    set(v) {
                        if(this.playabilityStatus
                            ?.errorScreen) {
                            delete this.playabilityStatus
                                .errorScreen;
                            this.playabilityStatus.status =
                                "OK"
                        }
                        return v
                    }
        ...
        function patchPlayerArgs(args, a) {
            ...
            if(args.adsense_video_doc_id) delete args
            ...
            if(args.csi_page_type) args.csi_page_type = args
                .csi_page_type.replace(/watch7ad/, "watch7");
            if(args.enable_csi) args.enable_csi = "0";
            if(args.pyv_ad_channel) delete args.pyv_ad_channel;
            if(args.show_pyv_in_related) args.show_pyv_in_related =
                false;
        ...
        function do1(ev, a) {
            ...
            if((a = document.scripts[document.scripts.length - 1]) &&
                /"adPlacements"/.test(a.text)) {
                a.text = a.text.replace(/"adPlacements"/,
                        ...
                        if(ytConfig) {
                            ...
                            if(a = ytConfig.EXPERIMENT_FLAGS) {
                                if(a
                                    ...
                                    false;
                                if(a
                                    .html5_check_both_ad_active_and_ad_info)
                                    a
                                    .html5_check_both_ad_active_and_ad_info =
                                    false;
                                if(a
                            ...
                            if(ytConfig.TIMING_ACTION) ytConfig
                                .TIMING_ACTION = ytConfig
                                .TIMING_ACTION.replace(/watch7ad/,
                                    "watch7");
                            ...
                            if(
                                ...
                                a.serializedExperimentFlags && a
                                .serializedExperimentFlags.replace
                            ) {
                                a.serializedExperimentFlags = a
                ...
                if(window.yt.player && window.yt.player.Application) {
                    if(window.yt.player.Application.create) {
                        if(!window.yt.player.Application.create.dyva) {
                            var ytPlayerApplicationCreate = window.yt
                                .player.Application.create;
                            window.yt.player.Application.create =
                                function(id, ytPlayerConfig) {
                                    if((id === "player-api") &&
                                        ytPlayerConfig && ytPlayerConfig
                                        .args) {
                                        if(ytPlayerConfig.args
                                            .raw_player_response)
                                            patchPlayerResponse(
                                                ytPlayerConfig.args
                                                .raw_player_response);
                                        if(ytPlayerConfig.args.vmap)
                                            delete ytPlayerConfig.args
                                            .vmap;
                                    }
                                    return ytPlayerApplicationCreate
                                        .apply(this, arguments);
                                };
                            window.yt.player.Application.create.dyva =
                                true;
                        }
                    }
                        ...
                        if(!window.yt.player.Application.createAlternate
                            ...
                            window.yt.player.Application
                                .createAlternate = function(id,
                                    ...
                                    return ytPlayerApplicationCreateAlternate
                                        .apply(this, arguments);
                                };
                ...
                if(window.yt.setConfig) {
                    if(!window.yt.setConfig.dyva) {
                        ...
                        window.yt.setConfig = function(ytConfig) {
                            if(ytConfig && ytConfig.ADS_DATA)
                                delete ytConfig.ADS_DATA;
                            return ytSetConfig.apply(this,
                                arguments);
                        };
            ...
            if(window.ytplayer && window.ytplayer.config && window
        ...
        addEventListener("spfpartprocess", function(ev) { //old youtube
            if(ev.detail && ev.detail.part && ev.detail.part
                .data &&
                ev.detail.part.data.swfcfg && ev.detail.part
                .data.swfcfg.args) {
                patchPlayerArgs(ev.detail.part.data.swfcfg
                .args);
            }
        }, true);
...
.ytp-autohide .caption-window.ytp-caption-window-bottom, .ytp-hide-controls .caption-window.ytp-caption-window-bottom{margin-bottom:0!important}`;
        ...
        document.createElement = function(tag) {
            if((tag === "meta") && window.ytInitialPlayerResponse &&
    ...
    if(("object" === typeof GM_info) && ["FireMonkey", "Greasemonkey"]
        .includes(GM_info.scriptHandler)) {
